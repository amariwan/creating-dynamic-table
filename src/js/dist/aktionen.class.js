class aktionenVM {
	constructor(url) {
		...
		self.init = async function () {
			try {
				// loaded data
				var agenten = await self.getJson('casa/uireq/casatable/clt/js/json/agenten.json');
				var agentenstatus = await self.getJson('casa/uireq/casatable/clt/js/json/agentenstatus.json');
				var aktionen = await self.getJson('casa/uireq/casatable/clt/js/json/aktionen.json');
				var aktionsagenten = await self.getJson('casa/uireq/casatable/clt/js/json/aktionsagenten.json');
				var produkte = await self.getJson('casa/uireq/casatable/clt/js/json/produkte.json');
				var json = {
					agenten: agenten,
		...
		self.createTable = (nodes, propertys) => {
			...
			self.columns(self.table.koColumnHeaders(propertys));
			self.rows(self.table.koRows(nodes));
		};
